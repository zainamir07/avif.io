...
export default function BlogPost() {
  ...
  return (
    ...
    >
      ...
      <br /> <b>The experimental format handles lossless images efficiently</b>
      ...
      <br />
      <H level={2} callback={callback} text="Speed" /> <b>In its development</b>
      , Google claims the format is{" "}
      <b>
      ...
      <br /> <b>AVIF supports parallelization</b>, which allows the image coder
      to take advantage of multiple cores and threads to improve encode and
      decode speeds. WebP's parallelization wasn't as efficient as AVIF's, so we
      expect that WebP2's gains in performance will be down to better use of
      ...
      <br /> <b>AVIF supports overlays and depth maps</b>. Overlays enable
      layers to help maintain the clarity of text and graphics over highly
      compressed photo backgrounds. Depth maps allow users to add effect to
      foregrounds and backgrounds.
      <br /> <b>These features were missing from the original WEBP</b>. At this
      stage in its successor's development cycle,{" "}
      <b>we don't know whether Google includes this.</b> <br />
      ...
      <br /> <b>Safari and Microsoft Edge are yet to support the standard</b>.
      Firefox users can only display AVIF images (though Firefox doesn't support
      animated AVIFs) if they enable it in their browser config. Although, with
      this moment, we do expect AVIF to reach universal support eventually.
      <br /> <b>WEBP2 is entirely experimental</b> and is not available to the
      public yet. <b>Understandably, no browser supports the image format.</b>{" "}
      ...
      <b>Most people will not be able to use WEBP2 for a long while.</b> It is
      highly experimental and won't be used for severe image delivery for quite
      some time. With no browser currently supporting the in-development image
      format, marketeers will not benefit from any experimental features Google
      is cooking up.
      <br /> <b>WEBP2 will significantly improve over its predecessor</b>, and
      when the format is released, we expect its features to impress. For
      lossless image delivery, marketers can benefit from much smaller file
      sizes than AVIF. However, for web delivery, AVIF will still hold the edge
      ...
      <b>
        AVIF is distinguishing itself as a fantastic replacement for JPEG
      </b>{" "}
      with its significant lossy compression.{" "}
      <b>We think AVIF has the spec sheet and the momentum </b> to become{" "}
      <b> the dominant image file format</b> for web delivery.
      <br />
    </Blog>
  );
}
