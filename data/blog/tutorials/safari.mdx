...
It took Apple more than ten years to implement WebP. For a long period, the browser only supported GIF, SVG, PNG images and the jpeg format, as well as their own HEIF codec. Since they are among the Alliance for Open Media companies actively developing AVIF, **we can expect them to release support faster than they did with WebP**. However, support for AV1, the video coding format that AVIF has derived from, is still pending as well.
