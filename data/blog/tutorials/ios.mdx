...
iOS was first released in 2007, along with the first iPhone. 2010 saw the development of an iPad OS version for tablets. Apple has since replaced this with iPadOS, amore open and modified version of iOS.
...
One of themobile browsers still unable to display AVIF images is Safari. AVIF images can be viewed by iOS users using third-party web browsers such as Chrome or Opera Mobile. Theproblem is that users can't view AVIF images downloaded from the Internet on their Apple iOS devices, since the device itself doesn't support AVIF decoding.
