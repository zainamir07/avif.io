...
A new Paint.NET update includes full support for AV1 (_.avif) images, courtesy of @null54's AvifFileType plugin, which has been bundled. In addition to opening AV1 (_.avif) images, @null54's AvifFileType plugin (v1.1.4.0) can also be saved (and notopened). There is no need to install Microsoft's codec from the Microsoft Store, which works on both Windows 7 and 8.1.

## How to open AVIF files

To view AVIF files, youimport them into the image viewer. To export an artboard as an AVIF,go Files -> Save As and choose AVIF as the image format. A prompt will appear with several settings you can choose to change the quality, speed of the compression, Chrome subsampling and some more. Press OK to save the file.
