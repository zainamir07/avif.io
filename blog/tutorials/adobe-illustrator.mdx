...
There are numerous enhancements and new features included in the May 2022 release of Adobe Illustrator for the desktop (version 26.3). AVIF files are now supported by Illustrator. The AVIF file format utilizes the AV1 compression algorithm. AV1 is an open file format originally designed for internet video transmission.
...
AVIF, which is a new image compression codec, offers a much smaller file size than JPEG, which has been used for over 20 years. This compression can be accomplished without sacrificing image quality or details in the image. Furthermore, it reduces the weight of an image by approximately half in comparison to a WebP.
